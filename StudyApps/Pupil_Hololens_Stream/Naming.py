...
def decompose_walkonly_file_name(filename: str):
    # T7_EU_B4_C7_S12345
    components = filename.split('_')
    target = components[0][1:]
    environment = components[1][1:]
    block = components[2][1:]
    C = components[3][1:]
    subnum = components[4][1:]
    return target, environment, block, C, subnum
...
def make_experiment_array_walkonly(sub_num):
    ...
    for block in range(len(EPblock)):
        for repetition in range(rep):
            ...
            for c in range(len(targets)):
                sendstring = 'T' + str(targets[c]) + '_E' + str(EPblock[block]) + '_B' + str(repetition) + '_C' + str(
                    c) + '_S' + str(sub_num)
    ...
    total_array.reverse()
    print('Total Trials count:', len(total_array))
    # print(total_array)
    return total_array
