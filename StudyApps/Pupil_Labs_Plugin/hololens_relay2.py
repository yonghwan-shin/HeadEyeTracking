...
class Hololens_Relay2(Plugin):
    ...
    def init_ui(self):
        self.add_menu()
        self.menu.label = "Hololens Relay2"
        self.update_menu()
    ...
    def thread_loop(self, context, pipe):
        ...
        ipc_sub = zmq_tools.Msg_Receiver(
            ...
            ("pupil", "notify.calibration.failed", "notify.calibration.successful"),
        )
                ...
                while ipc_sub.new_data:
                    ...
                    if (
                        ...
                        if method == "2d":
                            ...
                            # data = b"EG%s%s" % (
                            #     struct.pack("ff", *payload["norm_pos"]),
                            #     struct.pack("ffff",payload["phi"], payload["theta"],payload["confidence"],payload["timestamp"])
                            # )
                            bytesData = "EG" + str(payload["norm_pos"][0]) +","+ str(payload["norm_pos"][1]) +","+ str(payload["phi"]) +","+ str(payload["theta"]) + ","+str(payload["confidence"]) + ","+str(payload["timestamp"])
                            data = bytesData.encode()
                        else:
                            logger.error(
