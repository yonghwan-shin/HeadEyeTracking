        ...
        for target, env, pos, block in itertools.product(targets, envs, poss, blocks):
            ...
            try:
                ...
                df_eye.to_csv(
                    path_or_buf=(
                        DATA_ROOT
                        / "refined_eye_data_original"
                        / ("refined_" + eye_data.name)
                    ),
                    index=False,
                )
...
def summary_holo_data_proceed():
    ...
    df_output.to_csv(
        path_or_buf=(
            DATA_ROOT / "hololens_analysis" / "summary_1st_analysis_proceed.csv"
        ),
        index=False,
    )
...
def summary_holo_data():
    ...
    df_output.to_csv(
        path_or_buf=(DATA_ROOT / "hololens_analysis" / "summary_1st_analysis.csv"),
        index=False,
    )
    print("saved")
...
def analyse_holo_data():
                ...
                outputList.append(
                    [
                        ...
                        *current_info,
                        ...
                        head_rotation_y_min,
                        head_rotation_z_min,
                        head_rotation_x_range,
                        head_rotation_y_range,
    ...
    output.to_csv(
        path_or_buf=(DATA_ROOT / "hololens_analysis" / "1st_analysis.csv"), index=False
    )
...
def summary_eye_data():
    ...
    df_output.to_csv(
        path_or_buf=(DATA_ROOT / "refined_eye_data_analysis" / "1st_analysis.csv"),
        index=False,
    )
...
def get_final_dataset(_target, _env, _pos, _block, _subject):
    ...
    try:
        ...
        timestamps = {
            "imu": df_imu["IMUtimestamp"],
            "eye": df_eye["timestamp"],
            "holo": df_holo["Timestamp"],
        }
        ...
        for index, row in df_eye.iterrows():
            if index == 0:
                ...
                if (
                    ...
                    and df_eye.loc[index - 1, "check"] != "drop"
                ):
                    low_conf_start.append(df_eye.loc[index, "timestamp"])
                if (df_eye.loc[index, "check"] != "drop") and (
        ...
        [
            interpolation_eye_phi,
            interpolation_eye_the,
            interpolation_eye_x,
            interpolation_eye_y,
        ] = get_interpolation_function(
        ...
        [
            interpolation_holo_x,
            interpolation_holo_y,
            interpolation_holo_z,
            interpolation_holo_theta,
            interpolation_holo_phi,
        ] = get_interpolation_function(
            timestamps["holo"],
            df_holo["HeadRotationX"],
            df_holo["HeadRotationY"],
            df_holo["HeadRotationZ"],
            df_holo["Theta"],
            df_holo["Phi"],
        )
        interpolation_eye_phi_raw = interpolate.interp1d(
            timestamps["eye_conf"], df_eye_phi
        )
        interpolation_eye_the_raw = interpolate.interp1d(
            timestamps["eye_conf"], df_eye_the
        )
        ...
        dataframe = {
            ...
            "eye": df_eye,
            "holo": df_holo,
            "eye_conf": df_eye_conf,
            "eye_low_conf": df_eye_low_conf,
        }
        interpolations = {
            "imuX": interpolation_imu_x,
            "imuY": interpolation_imu_y,
            "imuZ": interpolation_imu_z,
            "holoX": interpolation_holo_x,
            "holoY": interpolation_holo_y,
            "holoZ": interpolation_holo_z,
            "holoPhi": interpolation_holo_phi,
            "holoThe": interpolation_holo_theta,
            "eyeX_raw": interpolation_eye_x_raw,
            "eyeY_raw": interpolation_eye_y_raw,
            "eyeX": interpolation_eye_x,
            "eyeY": interpolation_eye_y,
            "eyePhi_raw": interpolation_eye_phi_raw,
            "eyeThe_raw": interpolation_eye_the_raw,
            "eyePhi": interpolation_eye_phi,
            "eyeThe": interpolation_eye_the,
        ...
        low_conf = {
            "start": np.asarray(low_conf_start),
            "end": np.asarray(low_conf_end),
        }
...
def save_dataframe_as_one():
    ...
    for target, env, pos, block, subject in itertools.product(
        targets, envs, poss, blocks, subjects
    ):
        try:
            (
                x_axis,
                ...
                interpolations,
                current_info,
            ) = get_final_dataset(target, env, pos, block, subject)
            data = data.append(
                {
                    "subject": subject,
                    "target": target,
...
def target_positional_analysis():
    for subject in subjects:
        ...
        for target, env, pos, block in itertools.product(targets, envs, poss, blocks):
            try:
                ...
                for time in timeline:
                    t1 = np.where(
                        ((x_axis["test"] > time[0]) & (x_axis["test"] < time[1]))
                    )
                    t = np.arange(
                        math.ceil(time[0] * 100) / 100,
                        math.floor(time[1] * 100) / 100,
                        0.005,
                    )
                    ...
                    target_vertical = interpolations["holoX"](t) + interpolations[
                        "holoThe"
                    ](t)
                    target_horizontal = interpolations["holoY"](t) - interpolations[
                        "holoPhi"
                    ](t)
                    ...
                    if p_vertical > 0.05 or p_horizontal > 0.05:
                        continue
                    eye_linregress_vertical = (
                        slope_vertical * filtered_eye_vertical + intercept_vertical
                    )
                    eye_linregress_horizontal = (
                        slope_horizontal * filtered_eye_horizontal
                        + intercept_horizontal
                    )
                    eye_normalised_vertical = normalize(eye_vertical, target_vertical)

                    target_verticals = np.append(
                        target_verticals, filtered_target_vertical
                    )
                    target_horizontals = np.append(
                        target_horizontals, filtered_target_horizontal
                    )

                    eye_linregress_verticals = np.append(
                        eye_linregress_verticals, eye_linregress_vertical
                    )
                    eye_linregress_horizontals = np.append(
                        eye_linregress_horizontals, eye_linregress_horizontal
                    )
                    eye_filtered_verticals = np.append(
                        eye_filtered_verticals, filtered_eye_vertical
                    )
                    eye_filtered_horizontals = np.append(
                        eye_filtered_horizontals, filtered_eye_horizontal
                    )
                ...
                (
                    ...
                    p_vertical,
                    std_err_vertical,
                ) = stats.linregress(eye_filtered_verticals, target_verticals)
                (
                    slope_horizontal,
                    intercept_horizontal,
                    r_horizontal,
                    p_horizontal,
                    std_err_horizontal,
                ) = stats.linregress(eye_filtered_horizontals, target_horizontals)

                whole_dff_vertical = np.append(
                    whole_dff_vertical, target_verticals - eye_linregress_verticals
                )
                whole_dff_horizontal = np.append(
                    whole_dff_horizontal,
                    target_horizontals - eye_linregress_horizontals,
                )
                whole_target_vertical = np.append(
                    whole_target_vertical, target_verticals
                )
                whole_target_horizontal = np.append(
                    whole_target_horizontal, target_horizontals
                )
                whole_dff_norm_vertical = np.append(
                    whole_dff_norm_vertical, eye_normalised_vertical
                )
                whole_dff_norm_horizontal = np.append(
                    whole_dff_norm_horizontal, eye_normalised_vertical
                )
                whole_eye_vertical = np.append(
                    whole_eye_vertical, eye_filtered_verticals
                )
                whole_eye_horizontal = np.append(
                    whole_eye_horizontal, eye_filtered_horizontals
                )
                ...
                continue
        sns.scatterplot(
            whole_target_horizontal,
            whole_target_vertical,
            ax=axs[0],
            markers="+",
            alpha=0.3,
        )
        ...
        sns.scatterplot(
            whole_dff_horizontal, whole_dff_vertical, ax=axs[1], markers="+", alpha=0.3
        )
        confidence_ellipse(
            whole_dff_horizontal,
            whole_dff_vertical,
...
def bring_dataframe(imu_file_list, eye_file_list, hololens_file_list, current_info):
    # try:
    df_imu = manipulate_imu(
        file_as_pandas(get_file_by_info(imu_file_list, current_info))
    )
    ...
    check_holo_file(df_holo, current_info)
    df_imu["IMUtimestamp"] = (
        df_imu["IMUtimestamp"] - df_imu["IMUtimestamp"].head(1).values[0] - 0.05
    )
    df_eye["timestamp"] = (
        df_eye["timestamp"] - df_eye["timestamp"].head(1).values[0] - 0.05
    )
...
def slopes(data):
    ...
    for index, row in data.iterrows():
        ...
        for time in timeline:
            t1 = np.where(
                ((row["x_axis"]["test"] > time[0]) & (row["x_axis"]["test"] < time[1]))
            )
            if len(t1[0]) < 50:
                # print('too short timeline')
                continue
            t = np.arange(
                math.ceil(time[0] * 100) / 100, math.floor(time[1] * 100) / 100, 0.005
            )
            target_vertical = row["interpolations"]["holoX"](t) + row["interpolations"][
                "holoThe"
            ](t)
            target_horizontal = row["interpolations"]["holoY"](t) - row[
                "interpolations"
            ]["holoPhi"](t)
            ...
            (
                ...
                p_vertical,
                std_err_vertical,
            ) = stats.linregress(filtered_eye_vertical, filtered_target_vertical)
            (
                slope_horizontal,
                intercept_horizontal,
                r_horizontal,
                p_horizontal,
                std_err_horizontal,
            ) = stats.linregress(filtered_eye_horizontal, filtered_target_horizontal)
            if p_vertical > 0.05 or p_horizontal > 0.05:
                continue
